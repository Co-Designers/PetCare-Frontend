...
// ========== NUEVO: Endpoint de disponibilidad para clínicas ==========
server.get('/api/v1/clinics/:id/available-slots', (req, res) => {
  ...
  let startHour = 8, endHour = 20; // por defecto
  if (dayOfWeek === 'Saturday') { startHour = 9; endHour = 14; }
  if (dayOfWeek === 'Sunday') return res.json([]); // domingo cerrado
...
// ========== NUEVO: Endpoint de disponibilidad para profesionales móviles ==========
server.get('/api/v1/mobile-professionals/:id/available-slots', (req, res) => {
  ...
  const db = router.db;
  let availability = db.get('mobileAvailability')
  ...
  availability.forEach(block => {
    let startHour = parseInt(block.startTime.split(':')[0]);
    let endHour = parseInt(block.endTime.split(':')[0]);
    for (let hour = startHour; hour < endHour; hour++) {
      const startTime = `${hour.toString().padStart(2,'0')}:00`;
      if (!occupiedSlots.includes(startTime)) {
        availableSlots.push({ startTime, endTime: `${(hour+1).toString().padStart(2,'0')}:00` });
      }
    }
  });
  res.json(availableSlots);
...
// ========== APPOINTMENTS ==========
// POST /appointments con validación de disponibilidad
server.post('/api/v1/appointments', (req, res) => {
  const newApp = req.body;
  const db = router.db;

  // Validaciones básicas
  if (!newApp || !newApp.dateTime) return res.status(400).json({ error: 'Invalid appointment payload' });
  if (!newApp.providerType) return res.status(400).json({ error: 'providerType is required' });

  // Normalizar valores numéricos que pueden llegar como strings
  ...
  // Verificar existencia de pet si viene
  if (petId !== undefined) {
  ...
  if (isNaN(dateTime.getTime())) return res.status(400).json({ error: 'Invalid dateTime' });
  const dateStr = dateTime.toISOString().split('T')[0];
  const hour = dateTime.getHours();

  let finalClinicId = clinicId;
  if (newApp.providerType === 'clinic') {
    // permitir que el front envíe providerId como clinic identifier
    const resolvedClinicId = clinicId !== undefined && !isNaN(clinicId) ? clinicId : (providerId !== undefined && !isNaN(providerId) ? providerId : undefined);
    if (resolvedClinicId === undefined) return res.status(400).json({ error: 'clinicId or providerId is required for clinic appointments' });
    ...
    if (dayOfWeek === 'Saturday') { startHour = 9; endHour = 14; }
    if (dayOfWeek === 'Sunday') return res.status(400).json({ error: 'Clinic closed on Sunday' });

    // comprobar conflictos tanto por clinicId como por veterinarianId (si se proporciona)
    const appointments = db.get('appointments').filter(a => parseInt(a.clinicId) === resolvedClinicId && a.status !== 'cancelled').value();
    const occupiedByClinic = appointments.some(a => {
      const aDate = new Date(a.dateTime);
      return aDate.toISOString().split('T')[0] === dateStr && aDate.getHours() === hour;
    });
    if (occupiedByClinic) return res.status(409).json({ error: 'Time slot already taken' });

    if (newApp.veterinarianId) {
      const vetId = parseInt(newApp.veterinarianId);
      if (!isNaN(vetId)) {
        const vetAppointments = db.get('appointments').filter(a => parseInt(a.veterinarianId) === vetId && a.status !== 'cancelled').value();
        const occupiedByVet = vetAppointments.some(a => {
          const aDate = new Date(a.dateTime);
          return aDate.toISOString().split('T')[0] === dateStr && aDate.getHours() === hour;
        });
        if (occupiedByVet) return res.status(409).json({ error: 'Veterinarian not available at this time' });
      }
    }
  } else if (newApp.providerType === 'mobile') {
    if (providerId === undefined || isNaN(providerId)) return res.status(400).json({ error: 'providerId is required for mobile appointments' });
    const availability = db.get('mobileAvailability').filter(a => a.mobileId === providerId && a.date === dateStr).value();
    ...
    if (!withinAvailability) return res.status(400).json({ error: 'Outside working hours' });

    const accepted = db.get('mobileRequests').filter(r => r.mobileId === providerId && r.status === 'accepted').value();
    ...
    if (occupied) return res.status(409).json({ error: 'Time slot already taken' });
    // Además de crear una entrada en appointments, también crearemos una mobileRequest
    // para que el profesional móvil pueda verla en su panel (si la app móvil consulta /mobile-requests)
    // Preparamos el objeto mobileRequest que será creado más abajo junto con appointment
    // (si el front ya envió address/ownerId/serviceId los usaremos)
  ...
  const appointment = {
    id,
    ...newApp,
    // Asegurar clinicId cuando providerType sea 'clinic' y el front envíe providerId
    clinicId: (typeof finalClinicId !== 'undefined' && finalClinicId !== null)
      ? finalClinicId
      : (clinicId !== undefined ? clinicId : (providerId !== undefined ? providerId : newApp.clinicId)),
  ...
  // Si es mobile, crear también mobileRequest para que el profesional pueda verlo
...
// ========== SERVICE PROVIDERS (búsqueda unificada) ==========
...
server.listen(3000, () => {
  console.log('✅ JSON Server with custom routes is running on port 3000');
});
